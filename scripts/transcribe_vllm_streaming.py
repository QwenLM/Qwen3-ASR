...
Usage:
  ...
  --output/-o             JSON output path (default: results/<input_basename>.streaming.<model_name>.no-vad.no-aligner.json)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", "-o", default=None, help="JSON output path (default: results/<input_basename>.streaming.<model_name>.no-vad.no-aligner.json)")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            for ch in range(num_channels):
                ...
                if args.output:
                    ...
                    output_path = f"results/{basename}.channel{ch}.streaming.{model_name}.no-vad.no-aligner.json"

                _stream_transcribe(asr, wav16k, args, model_load_s, args.input, output_path)
    else:
        output_path = args.output or f"results/{basename}.streaming.{model_name}.no-vad.no-aligner.json"
